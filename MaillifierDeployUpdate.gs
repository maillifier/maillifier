...
function fetchFileList_() {
  // Read manifest.json from raw.githubusercontent.com (no API rate limit).
  // Manifest is generated by the CI/CD pipeline and contains the list of
  // files with their git blob SHA-1 hashes.
  var pathSegment = DEPLOY_CONFIG.PATH ? DEPLOY_CONFIG.PATH + '/' : '';
  var manifestUrl = 'https://raw.githubusercontent.com/' + DEPLOY_CONFIG.OWNER + '/' +
    DEPLOY_CONFIG.REPO + '/' + DEPLOY_CONFIG.BRANCH + '/' + pathSegment + 'manifest.json';

  var response = UrlFetchApp.fetch(manifestUrl, { muteHttpExceptions: true });

  if (response.getResponseCode() !== 200) {
    throw new Error('Failed to fetch manifest.json (' + response.getResponseCode() + '): ' +
  ...
  // Normalize manifest entries to the shape expected by downloadAndVerify_.
  return manifest.files
    .filter(function(item) {
      return item.name.endsWith('.gs') || item.name === 'appsscript.json';
    })
    .map(function(item) {
      return { name: item.name, sha: item.sha };
    });
}

function fetchFileContent_(file) {
  // Download the file via raw.githubusercontent.com (no API rate limit).
